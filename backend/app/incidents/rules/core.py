...
def classify_incident_severity(
    ...
    if (downtime_minutes >= 60 and len(services_critical) >= 1) or \
       (clients_affected >= 1000) or \
...
def calculate_deadlines(
    ...
    initial_deadline, initial_dst = _add_hours_with_dst(anchor_brussels, initial_hours, tz)
    ...
    if deadline_config["intermediate_hours"] is not None:
        intermediate_deadline, intermediate_dst = _add_hours_with_dst(
            anchor_brussels, 
            deadline_config["intermediate_hours"], 
            tz
        )
    ...
    nbb_deadline = initial_deadline if severity in [Severity.MAJOR, Severity.CRITICAL] else None
    
    return Success(DeadlineCalculation(
        ...
        intermediate_report=intermediate_deadline.astimezone(timezone.utc) if intermediate_deadline else None,
        final_report=final_deadline.astimezone(timezone.utc),
        nbb_notification=nbb_deadline.astimezone(timezone.utc) if nbb_deadline else None,
...
def _is_spring_forward_gap(dt: datetime, tz: ZoneInfo) -> bool:
    ...
    return (
        dt.month == 3 and 
        dt.day == march_last_sunday and
        dt.hour == 2  # This hour doesn't exist during spring forward
    )
