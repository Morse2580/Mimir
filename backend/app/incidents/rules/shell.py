...
async def classify_and_persist(incident_id: str) -> Result[ClassificationResult, str]:
    ...
    try:
        ...
        if isinstance(anchor_result, Failure):
            await _emit_event(InvalidClockAnchor(
        ...
        if severity != Severity.NO_REPORT:
            deadline_result = calculate_deadlines(anchor_timestamp, severity, BRUSSELS_TZ)
        ...
        requires_notification = severity in [Severity.CRITICAL, Severity.MAJOR, Severity.SIGNIFICANT]
        ...
        if deadlines:
            ...
            for transition in deadlines.dst_transitions_handled:
                await _emit_event(DSTTransitionHandled(
                    incident_id=incident_id,
...
async def schedule_notifications(
    ...
    try:
        ...
        error_msg = f"Notification scheduling failed for incident {incident_id}: {str(e)}"
        await _log_error(error_msg, incident_id)
        return Failure(error_msg)
...
def _build_classification_reasons(incident: IncidentInput, severity: Severity) -> tuple[str, ...]:
    ...
    if severity == Severity.MAJOR:
        if incident.downtime_minutes >= 60 and len(incident.services_critical) >= 1:
            reasons.append(f"Downtime {incident.downtime_minutes}min >= 60min with {len(incident.services_critical)} critical services")
        ...
        if "payment" in incident.services_critical and incident.downtime_minutes >= 30:
            reasons.append(f"Payment service affected with {incident.downtime_minutes}min downtime >= 30min")
    
    elif severity == Severity.SIGNIFICANT:
        if 100 <= incident.clients_affected < 1000:
            reasons.append(f"Clients affected {incident.clients_affected} in range 100-999")
        
        if 15 <= incident.downtime_minutes < 60 and len(incident.services_critical) >= 1:
            reasons.append(f"Downtime {incident.downtime_minutes}min in range 15-59min with {len(incident.services_critical)} critical services")
