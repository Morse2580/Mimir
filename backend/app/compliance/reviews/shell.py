...
from .contracts import (
    ...
    Reviewer, AuditTrailEntry, AuditAction, ChainVerificationResult,
    ReviewAuditReport
)
from .core import (
    ...
    can_transition_status, build_audit_trail_entry, verify_hash_chain,
    validate_reviewer_capacity, calculate_review_metrics
)
from .events import (
    ReviewRequested, ReviewAssigned, ReviewStarted, DecisionRecorded,
    MappingMarkedStale, ReviewSLABreached, AuditTrailViolation,
...
class ReviewWorkflowService:
    ...
    async def record_decision(
        ...
        if not can_transition_status(current_status, decision):
            raise ValueError(f"Invalid status transition from {current_status} to {decision}")
        ...
        if is_mapping_stale(request.mapping_version_hash, current_mapping):
            ...
            raise ValueError("Mapping has changed since review started - marked as stale")
    ...
    async def check_sla_breaches(self) -> List[ReviewSLABreached]:
        ...
        for request, reviewer_id in active_reviews:
            if is_sla_breached(request.submitted_at, request.priority, current_time):
                ...
                deadline = calculate_sla_deadline(request.submitted_at, request.priority)
    ...
    async def verify_audit_chain_integrity(self) -> ChainVerificationResult:
        ...
        event = ChainIntegrityVerified(
            ...
            last_entry_hash=audit_entries[-1].context_data.get("chain_hash", "") if audit_entries else "",
    ...
    async def export_audit_trail(
        ...
        report = ReviewAuditReport(
            ...
            reviews_by_status={ReviewStatus(k): v for k, v in metrics["reviews_by_status"].items()},
            ...
            reviewers_activity=await self._get_reviewer_activity_stats(start_date, end_date)
        )
    ...
    async def _update_assignment(self, request_id: str, reviewer_id: str, assigned_by: str) -> None:
        """Update review assignment."""
        # Mock implementation
        pass
    
    async def _update_review_status(self, request_id: str, status: ReviewStatus) -> bool:
    ...
    async def _notify_reviewer_assigned(self, reviewer: Reviewer, request: ReviewRequest) -> None:
