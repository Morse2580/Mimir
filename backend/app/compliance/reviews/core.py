...
from .contracts import (
    ReviewStatus, ReviewPriority, ReviewRequest, ReviewDecision,
    AuditTrailEntry, AuditAction, ChainVerificationResult, Reviewer
)
...
def determine_review_priority(
    ...
    if regulatory_deadline and current_time:
        hours_until_deadline = (regulatory_deadline - current_time).total_seconds() / 3600
...
def validate_reviewer_capacity(
    reviewer: Reviewer,
    ...
    if current_workload >= reviewer.workload_capacity:
        return Failure(f"Reviewer {reviewer.id} at capacity ({current_workload}/{reviewer.workload_capacity})")
...
def calculate_review_metrics(
    ...
    total_duration = sum(decision.review_duration_minutes for _, decision in completed_reviews)
    avg_duration = total_duration / len(completed_reviews)
    
    # Calculate SLA breach rate
    breached_count = 0
    for request, decision in completed_reviews:
        if decision and is_sla_breached(request.submitted_at, request.priority, decision.reviewed_at):
            breached_count += 1
