...
class TestPIIBoundaryProtection:
    ...
    def test_audit_trail_entry_sanitizes_context(self, pii_test_data):
        ...
        for pattern in pii_patterns:
            assert pattern not in str(context), f"PII pattern {pattern} found in audit context"
...
class TestPIIInjectionAttacks:
    ...
    @pytest.mark.asyncio
    async def test_pii_injection_in_rationale_blocked(self, review_service_with_pii_protection):
        """Test that PII in review rationale is blocked."""
        
        service = review_service_with_pii_protection
        
        # Mock dependencies
        service._get_mapping = AsyncMock(return_value={
            "mapping_id": "map_123",
            "obligation_id": "DORA_ART_18",
            "control_id": "C001",
    ...
    @pytest.mark.asyncio 
    async def test_pii_injection_in_comments_blocked(self, review_service_with_pii_protection):
    ...
    def test_pii_detection_patterns(self, review_service_with_pii_protection):
        ...
        for test_case in pii_test_cases:
            assert service._contains_pii(test_case), f"Failed to detect PII in: {test_case}"
        ...
        for test_case in safe_test_cases:
            assert not service._contains_pii(test_case), f"False positive PII detection in: {test_case}"
    ...
    async def test_reviewer_authorization_check(self):
        ...
        def check_reviewer_authorization(reviewer: Reviewer, action: str) -> bool:
            ...
            if action in required_certs:
                return any(cert in reviewer.certifications for cert in required_certs[action])
            return False
