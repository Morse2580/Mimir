...
class TestSubmitForReview:
    ...
    async def test_submit_for_review_success(
        self, review_service, mock_event_publisher, sample_mapping
    ):
...
class TestAssignReviewer:
    ...
    async def test_assign_reviewer_success(
        self, review_service, mock_event_publisher, sample_reviewer
    ):
    ...
    @pytest.mark.asyncio
    async def test_assign_reviewer_at_capacity(self, review_service, sample_reviewer):
...
class TestRecordDecision:
    ...
    async def test_record_decision_approved(
        self, review_service, mock_event_publisher, sample_reviewer, sample_mapping
    ):
        ...
        # Mock dependencies
        review_service._get_review_request = AsyncMock(return_value=request)
        review_service._get_current_status = AsyncMock(
            return_value=ReviewStatus.IN_REVIEW
        )
        review_service._get_mapping = AsyncMock(return_value=sample_mapping)
        review_service._store_review_decision = AsyncMock()
        review_service._update_review_status = AsyncMock()
        review_service._append_audit_entry = AsyncMock(
            return_value=MagicMock(id="audit_123")
        )
    ...
    async def test_record_decision_mapping_stale(
        self, review_service, sample_reviewer, sample_mapping
    ):
        ...
        review_service._get_mapping = AsyncMock(
        ...
        with pytest.raises(
            ValueError, match="Mapping has changed since review started"
        ):
    ...
    async def test_record_decision_invalid_transition(
        self, review_service, sample_reviewer
    ):
...
class TestSLABreachDetection:
    ...
    @pytest.mark.asyncio
    async def test_check_sla_breaches(self, review_service, mock_event_publisher):
        ...
        review_service._get_active_reviews = AsyncMock(
            return_value=[(overdue_request, "lawyer_001")]
        )
        review_service._get_current_status = AsyncMock(
            return_value=ReviewStatus.IN_REVIEW
        )
...
class TestAuditChainVerification:
    ...
    async def test_verify_audit_chain_integrity_valid(
        ...
        from ..contracts import AuditTrailEntry
        ...
        entry2 = AuditTrailEntry.create(
            ...
            evidence_ref="req_1",
            context_data={
                "chain_hash": "valid_hash_2",
                "previous_hash": "valid_hash_1",
            },
        )
        ...
        with patch(
            "backend.app.compliance.reviews.shell.verify_hash_chain"
        ) as mock_verify:
            mock_verify.return_value = ChainVerificationResult(
                valid=True, total_entries=2, verified_entries=2
            )
...
class TestAuditTrailExport:
    ...
    @pytest.mark.asyncio
    async def test_export_audit_trail(self, review_service):
        ...
        review_service._get_reviewer_activity_stats = AsyncMock(
            return_value={
                "lawyer_001": {
                    "reviews_completed": 5,
