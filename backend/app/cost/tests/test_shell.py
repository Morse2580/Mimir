...
class TestCostTracker:
    ...
    async def test_check_budget_kill_switch_already_active(
        self, cost_tracker, mock_redis
    ):
        """Test pre-flight check when kill switch is already active."""
        mock_redis.get.return_value = b"1"  # Kill switch active

        result = await cost_tracker.check_budget_before_call("search", "pro", "tenant1")
    ...
    async def test_check_budget_would_exceed_threshold(self, cost_tracker, mock_redis):
        ...
        result = await cost_tracker.check_budget_before_call("task", "pro", "tenant1")

        # This should be allowed since €1400.05 < €1425
        assert result.allowed is True
        assert result.projected_spend_eur == Decimal("1400.050")

    async def test_check_budget_exceeds_kill_switch_threshold(
        self, cost_tracker, mock_redis
    ):
    ...
    async def test_record_api_cost_success(self, cost_tracker, mock_redis, mock_db):
        ...
        with patch.object(
            cost_tracker, "_emit_event", new_callable=AsyncMock
        ) as mock_emit:
            with patch.object(
                cost_tracker, "_check_threshold_alerts", new_callable=AsyncMock
            ) as mock_alerts:
...
class TestKillSwitchOverride:
    ...
    async def test_manual_override_not_active(self, mock_redis):
        ...
        result = await manual_kill_switch_override(
            mock_redis, "tenant1", "ceo@company.com", "Test reason", "c_level"
        )
...
class TestErrorHandling:
    ...
    async def test_database_error_handling(self, cost_tracker, mock_redis, mock_db):
        ...
        with pytest.raises(Exception, match="Database error"):
            await cost_tracker.record_api_cost("search", "pro", "tenant1", "test")
