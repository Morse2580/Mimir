...
def with_cost_tracking(api_type: str, processor: str, use_case: str = "default"):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            preflight = await cost_tracker.check_budget_before_call(
                api_type=api_type, processor=processor, tenant=tenant, use_case=use_case
            )

            if not preflight.allowed:
                raise BudgetExceededException(
                    f"Budget limit exceeded: {preflight.reason}", preflight
                )
...
async def assert_parallel_safe(
    ...
    preflight = await cost_tracker.check_budget_before_call(
        api_type=api_type, processor=processor, tenant=tenant
    )

    if not preflight.allowed:
        raise BudgetExceededException(
            f"Budget check failed: {preflight.reason}", preflight
        )
