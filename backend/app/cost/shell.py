...
class CostTracker:
    ...
    async def check_budget_before_call(
        self, api_type: str, processor: str, tenant: str, use_case: str = "default"
    ) -> PreFlightCheck:
        """
    ...
    async def record_api_cost(
        ...
        await self._check_threshold_alerts(
            tenant, new_total_spend - cost_eur, new_total_spend
        )
    ...
    async def _activate_kill_switch(
        self, tenant: str, current_spend: Decimal, proposed_cost: Decimal
    ) -> None:
    ...
    async def _check_threshold_alerts(
        self, tenant: str, previous_spend: Decimal, current_spend: Decimal
    ) -> None:
    ...
    async def _emit_event(self, event) -> None:
        ...
        logger.info(
            f"Emitted event: {event.__class__.__name__} for {getattr(event, 'tenant', 'unknown')}"
        )


async def reset_monthly_budget(
    redis_client: redis.Redis, tenant: str, month: Optional[str] = None
) -> None:
    ...
    previous_spend = (
        Decimal(previous_spend_str.decode()) if previous_spend_str else Decimal("0.00")
    )
...
async def manual_kill_switch_override(
    ...
    current_spend = (
        Decimal(current_spend_str.decode()) if current_spend_str else Decimal("0.00")
    )
