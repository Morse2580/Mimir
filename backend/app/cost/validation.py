...
from .integration import assert_parallel_safe
...
async def validate_pii_boundaries():
    ...
    mock_tracker.check_budget_before_call.return_value.proposed_cost_eur = Decimal(
        "0.001"
    )
    ...
    for payload in safe_test_cases:
        try:
            await assert_parallel_safe(mock_tracker, payload, "test_tenant")
            print(
                f"✅ SAFE PAYLOAD: {payload['query'] if 'query' in payload else list(payload.values())[0]}"
            )
        except Exception as e:
...
def validate_kill_switch_accuracy():
    ...
    test_cases = [
        ...
        (
            ...
            Decimal("50.00"),
            True,
            "€1400 + €50 = €1450 > €1425 threshold",
        ),
        (
            Decimal("1400.00"),
            Decimal("25.00"),
            False,
            "€1400 + €25 = €1425 = threshold",
        ),
        (
            Decimal("1400.00"),
            Decimal("24.99"),
            False,
            "€1400 + €24.99 = €1424.99 < threshold",
        ),
        (
            Decimal("1424.99"),
            Decimal("0.01"),
            False,
            "€1424.99 + €0.01 = €1425.00 = threshold",
        ),
        (
            Decimal("1425.00"),
            Decimal("0.001"),
            True,
            "€1425 + €0.001 = €1425.001 > threshold",
        ),
        (
            Decimal("0.00"),
            Decimal("1425.00"),
            False,
            "€0 + €1425 = €1425.00 = threshold",
        ),
        (
            Decimal("0.00"),
            Decimal("1425.01"),
            True,
            "€0 + €1425.01 = €1425.01 > threshold",
        ),
    ]
    ...
    for (api_type, processor), expected_cost in expected_costs.items():
        try:
            ...
            if actual_cost == expected_cost:
                ...
                print(
                    f"❌ {api_type}:{processor} = €{actual_cost} (expected €{expected_cost})"
                )
                errors += 1
        except Exception as e:
