...
API_COSTS: Dict[str, Dict[str, Decimal]] = {
    "search": {"base": Decimal("0.001"), "pro": Decimal("0.005")},
    "task": {
        "base": Decimal("0.010"),
        "core": Decimal("0.020"),
        "pro": Decimal("0.050"),
...
def calculate_api_cost(api_type: str, processor: str, request_size: int = 0) -> Decimal:
    """
    ...
    if processor not in API_COSTS[api_type]:
        ...
        raise ValueError(
            f"Invalid processor '{processor}' for api_type '{api_type}'. Valid: {valid_processors}"
        )
...
def calculate_budget_utilization(
    current_spend: Decimal, monthly_cap: Decimal = MONTHLY_CAP_EUR
) -> Decimal:
    """
...
def get_threshold_amount(
    threshold_percent: Decimal, monthly_cap: Decimal = MONTHLY_CAP_EUR
) -> Decimal:
    """
